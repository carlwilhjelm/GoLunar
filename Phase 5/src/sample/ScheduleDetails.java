...
connect = SqliteConnection.Connector();
            ...
            if (rs.getInt(8) != 0) {
                ...
                ID1subjectName = new SimpleStringProperty(getSubjectDB(classID1subjectID) + " " + rs.getString(3));


                //retrieve Employee name from EmployeeTable
                int classID1employeeID = rs.getInt(4);
                ID1instructor = new SimpleStringProperty(getInstructorName(classID1employeeID));
            ...
            //get results set of student, then find class names, set variables etc
            result2 = statement.executeQuery("SELECT * FROM StudentTable where " + "StudentID = '" + studentIDin + "'");

            if (result2.getInt(10) != 0) {
                ...
                ID2subjectName = new SimpleStringProperty(getSubjectDB(classID2subjectID) + " " + result2.getString(3));


                //retrieve Employee name from EmployeeTable
                int classID2employeeID = result2.getInt(4);
                ID2instructor = new SimpleStringProperty(getInstructorName(classID2employeeID));
