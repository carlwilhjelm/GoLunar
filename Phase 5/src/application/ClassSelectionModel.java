...
public class ClassSelectionModel {
	public static char[] daysArr = new char[7];
	...
	public void setQueryControl(char ch, String key) throws SQLException {
		...
		switch (ch) {
			...
			case 'o':
				chi = key.charAt(0);
				queryControlArr[14] = waDays + singleQuotes(daysBuilder(true, chi));
				break;
			//days of the week removed
			case 'p':
				chi = key.charAt(0);
				String days = daysBuilder(false, chi);
				if (days.equals("")) { 
					queryControlArr[14] = null;
				}
				else {
					queryControlArr[14] = waDays + singleQuotes(days);
				}
				break;
			//reset
			case 'z':
	...
	/**
	 *precondition: takes input from query control method 
	 *postcondition: creates a string from selected days
	 */
	public String daysBuilder(boolean adding, char chIn) {
		int i = 0;
		
		//index starts with m = 0;
		switch (chIn) {
			case 'U': i++;
			case 'S': i++;
			case 'F': i++;
		...
		if (adding) {
			daysArr[i] = chIn;
			System.out.println("added " + chIn + " to index " + i);
		}
		else {
			daysArr[i] = 0;
			System.out.println("removed " + chIn + " from index " + i);
		}
	...
	*/
	...
	public ObservableList<ClassDetails> search() throws SQLException {
		...
		try {
			if (!buildQueryTest()) return options;
	...
	/*
	 * Test if one but not all time fields have been selected
	 */
	public boolean buildQueryTest() {
		boolean result = true;
		//if any of them are null but not all of them, error
		if (
				((queryControlArr[8] == null) 
				|| (queryControlArr[9] == null)
				|| (queryControlArr[10] == null)) 
				&& 
				!((queryControlArr[8] == null)
				&& (queryControlArr[9] == null)
				&& (queryControlArr[10] == null))
				...
				{
			System.out.println("Insufficient selection for start time, all fields must be selected");
			result = false;
		} 
		if (
				((queryControlArr[11] == null) 
				|| (queryControlArr[12] == null)
				|| (queryControlArr[13] == null)) 
				&& 
				!((queryControlArr[11] == null)
				&& (queryControlArr[12] == null)
				&& (queryControlArr[13] == null))
				...
				{
			System.out.println("Insufficient selection for end time, all fields must be selected");
			result = false;
		}
		if (result) {
			System.out.println("Sufficient time selection");
		}
		return result;
	}
